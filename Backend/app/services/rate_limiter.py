...
def policy_for_path(path: str) -> RateLimitPolicy:
    ...
    if path.startswith("/sos/trigger") or path.startswith("/emergency/bundle"):
...
class RateLimitMiddleware(BaseHTTPMiddleware):
    async def dispatch(self, request: Request, call_next):
        ...
        if not ok:
            # Standardised 429 body — Flutter reads retry_after_seconds for countdown timers.
            # Retry-After header is the HTTP standard; mobile clients use it for backoff.
